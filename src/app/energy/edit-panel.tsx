      ...
      <div className="fixed top-4 right-4 bottom-4 z-50 w-full max-w-md animate-slide-in-right flex flex-col overflow-hidden rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 shadow-2xl">
        <div className="shrink-0 flex items-center justify-between p-5 pb-3 border-b border-gray-200 dark:border-white/10">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100">
            {editingWidget.type === "text_card" ? t("editPanel.editText") : editingWidget.type === "power_usage_card" ? t("editPanel.editPowerUsage") : editingWidget.type === "device_consumption_card" ? t("editPanel.editDeviceConsumption") : t("editPanel.editTile")}
          </h3>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5 pt-4 space-y-3">
          {(editingWidget.type === "text_card") ? (
            ...
            <>
              ...
              <EntitySelectWithSearch entities={entities} value={editForm.entity_id} onChange={(v) => setEditForm((prev) => ({ ...prev, entity_id: v }))} filter={(e) => e.entity_id.startsWith("sensor.")} label={t("editPanel.solarChartEntity")} placeholder={t("editPanel.searchEntity")} emptyOption={t("editPanel.none")} />
              <EntitySelectWithSearch entities={entities} value={editForm.yield_entity_id_today ?? ""} onChange={(v) => setEditForm((prev) => ({ ...prev, yield_entity_id_today: v || undefined }))} filter={(e) => e.entity_id.startsWith("sensor.")} label={t("editPanel.solarTodayEntity")} placeholder={t("editPanel.searchSensor")} emptyOption={t("editPanel.none")} />
              <EntitySelectWithSearch entities={entities} value={editForm.yield_entity_id_month ?? ""} onChange={(v) => setEditForm((prev) => ({ ...prev, yield_entity_id_month: v || undefined }))} filter={(e) => e.entity_id.startsWith("sensor.")} label={t("editPanel.solarMonthEntity")} placeholder={t("editPanel.searchSensor")} emptyOption={t("editPanel.none")} />
            </>
            ...
            <>
              ...
              {editTab === "achtergrond" && (
                <>
                  <div>
                    ...
                    <input type="text" value={editForm.background_image ?? ""} onChange={(e) => setEditForm((prev) => ({ ...prev, background_image: e.target.value || undefined }))} placeholder={t("editPanel.orPasteUrl")} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 placeholder-gray-500 dark:border-white/10 dark:bg-white/5 dark:text-gray-200 dark:placeholder-gray-500" />
                  </div>
                  <div>
                    ...
                    <input type="text" value={editForm.background_image_dark ?? ""} onChange={(e) => setEditForm((prev) => ({ ...prev, background_image_dark: e.target.value || undefined }))} placeholder={t("editPanel.darkSameAsLight")} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 placeholder-gray-500 dark:border-white/10 dark:bg-white/5 dark:text-gray-200 dark:placeholder-gray-500" />
                  </div>
              ...
              {editTab === "voorwaarden" && (
                <div>
                  <label className="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">{t("editPanel.imageConditions")}</label>
                  {(editForm.image_conditions ?? []).map((cond, idx) => (
                    <div key={idx} className="flex flex-wrap items-start gap-2 rounded-lg border border-gray-200 dark:border-white/10 p-2 mb-2">
                      <select value={cond.operator} onChange={(e) => setEditForm((prev) => ({ ...prev, image_conditions: (prev.image_conditions ?? []).map((c, i) => i === idx ? { ...c, operator: e.target.value } : c) }))} className="rounded border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 dark:text-gray-200 px-2 py-1 text-xs">
                        {SENSOR_CONDITION_OPERATORS.map((op) => <option key={op} value={op}>{op === "contains" ? t("editPanel.operatorContains") : (SENSOR_CONDITION_OPERATOR_LABELS[op] ?? op)}</option>)}
                      </select>
                      <input type="text" value={cond.value} onChange={(e) => setEditForm((prev) => ({ ...prev, image_conditions: (prev.image_conditions ?? []).map((c, i) => i === idx ? { ...c, value: e.target.value } : c) }))} placeholder={t("editPanel.value")} className="flex-1 min-w-[80px] rounded border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 dark:text-gray-200 px-2 py-1 text-xs" />
                  ...
                  <button type="button" onClick={() => setEditForm((prev) => ({ ...prev, image_conditions: [...(prev.image_conditions ?? []), { operator: "eq", value: "", image: "", image_dark: undefined }] }))} className="rounded-lg border border-dashed border-gray-300 dark:border-white/20 px-3 py-2 text-xs text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5 w-full">{"+ " + t("editPanel.addConditionShort")}</button>
                </div>
              )}
            </>
            ...
            <>
              <div>
                <label className="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">{t("editPanel.name")}</label>
                <input type="text" value={editForm.title} onChange={(e) => setEditForm((prev) => ({ ...prev, title: e.target.value }))} placeholder={t("cardType.device_consumption_card")} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 placeholder-gray-500 dark:border-white/10 dark:bg-white/5 dark:text-gray-200 dark:placeholder-gray-500" />
              </div>
              <div>
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        {filtered.length === 0 && <p className="text-xs text-gray-500 py-2">{q ? t("editPanel.noSearchResultsTry") : t("editPanel.noSensorsFound")}</p>}
                      </>
                    );
                  })()}
                </div>
              ...
              {(editForm.device_entity_ids ?? []).length > 0 && (
                <div>
                  ...
                  <p className="mb-2 text-xs text-gray-500 dark:text-gray-400">{t("editPanel.deviceDisplayNamesHint")}</p>
            ...
            <div>
              ...
              {editTab === "algemeen" && (
                <>
                  ...
                  <label className="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">{t("editPanel.icon")}</label>
                  <input type="text" value={sensorIconSearch} onChange={(e) => setSensorIconSearch(e.target.value)} onFocus={() => setSensorIconSearch(sensorIconSearch || (editForm.icon ?? ""))} placeholder={t("editPanel.searchIcon")} className="mb-2 w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 placeholder-gray-500 dark:border-white/10 dark:bg-white/5 dark:text-gray-200 dark:placeholder-gray-500" />
                  <div className="max-h-40 overflow-auto rounded-lg border border-gray-200 dark:border-white/10 flex flex-wrap gap-1.5 p-1.5">
                    <button type="button" onClick={() => { setEditForm((prev) => ({ ...prev, icon: undefined })); setSensorIconSearch(""); }} className={cn("rounded-md px-2 py-1 text-xs", !editForm.icon ? "bg-[#4700B5] text-white" : "bg-gray-100 text-gray-700 dark:bg-white/10 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-white/20")}>Default (Gauge)</button>
                    {SENSOR_ICON_OPTIONS.filter((name) => name.toLowerCase().includes((sensorIconSearch || "").toLowerCase())).map((name) => (
                      <button key={name} type="button" onClick={() => { setEditForm((prev) => ({ ...prev, icon: name })); setSensorIconSearch(name); }} className={cn("rounded-md px-2 py-1 text-xs", (editForm.icon ?? "Gauge") === name ? "bg-[#4700B5] text-white" : "bg-gray-100 text-gray-700 dark:bg-white/10 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-white/20")}>{name}</button>
                    ))}
                  </div>
                  <div className="mt-3"><label className="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">{t("editPanel.size")}</label><select value={editForm.size ?? "md"} onChange={(e) => setEditForm((prev) => ({ ...prev, size: e.target.value }))} className="w-full rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 dark:border-white/10 dark:bg-white/5 dark:text-gray-200"><option value="sm">{t("editPanel.sizeSm")}</option><option value="md">{t("editPanel.sizeMd")}</option><option value="lg">{t("editPanel.sizeLg")}</option></select></div>
                  <div className="mt-3 flex items-center gap-2"><input type="checkbox" id="sensor-show-icon" checked={editForm.show_icon !== false} onChange={(e) => setEditForm((prev) => ({ ...prev, show_icon: e.target.checked }))} className="rounded border-gray-300 text-amber-600 focus:ring-amber-500" /><label htmlFor="sensor-show-icon" className="text-sm text-gray-700 dark:text-gray-300">{t("editPanel.showIcon")}</label></div>
                </>
              )}
              {editTab === "voorwaarden" && (
                <div>
        ...
        <div className="shrink-0 flex justify-between gap-2 p-5 pt-4 pb-6 border-t border-gray-200 dark:border-white/10 bg-white dark:bg-gray-900">
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => { const updates = getSaveUpdates(); handleUpdateTile(editingWidget.id, updates); const newWidgets = widgets.map((w) => (w.id === editingWidget.id ? { ...w, ...updates } : w)); saveMutation.mutate({ layout, widgets: newWidgets, welcomeTitle, welcomeSubtitle }); setEditingWidgetId(null); }} className="rounded-lg bg-[#4700B5] px-3 py-1.5 text-sm text-white hover:opacity-90">{t("editPanel.save")}</button>
          </div>
        </div>
      </div>
    </>
