...
import { Home, Pause, Play, Square, X } from "lucide-react";
...
import { normalizeSegmentId } from "@/lib/valetudo-map";
import {
...
export function VacuumControlView({
  ...
  const session = useVacuumSession({
    active: true,
    includeMaintenance: !sheet,
  });
  const {
    configured,
    map,
  ...
  const statusLine =
    loaded && configured ? (
      <p className={cn("text-sm text-gray-500 dark:text-gray-400", sheet ? "mt-0.5" : "mt-1")}>
        {t(statusKey)}
  ...
  const footer = map ? (
    <div
      className={cn(
        "flex flex-wrap items-center justify-between gap-3 rounded-card border border-white/60 bg-white/50 px-4 py-3 dark:border-white/10 dark:bg-white/5",
        sheet && "shrink-0"
      )}
    >
      <div className="min-w-0 flex-1 space-y-2">
        ...
        {sheet && rooms.length > 0 ? (
          <div className="flex flex-wrap gap-1.5">
            {rooms.map((room) => {
              const id = normalizeSegmentId(room.metaData?.segmentId);
              if (!id) return null;
              const name = segmentNameFromLayers(map.layers ?? [], id);
  ...
  const body = !configured ? (
    ...
    <>
      <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden lg:flex-row">
        <div className={cn("relative min-h-[32vh] flex-1 overflow-hidden", sheet && "min-h-0")}>
  ...
  if (sheet) {
    return (
      <div className="flex h-full min-h-0 flex-1 flex-col gap-3 px-4 pb-4 pt-2">
        <div className="flex items-start justify-between gap-3">
          ...
          {onClose ? (
            <button
              type="button"
              onClick={onClose}
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-black/10 text-gray-800 dark:bg-white/10 dark:text-white"
