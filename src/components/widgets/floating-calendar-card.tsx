...
const STORAGE_KEY_PREFIX = "dashboard.floatingCalendarCard.";
...
function defaultPosition(): Position {
  ...
  return {
    left: Math.max(DEFAULT_OFFSET, window.innerWidth - CARD_WIDTH - 32),
    bottom: 96,
  };
}
...
export function FloatingCalendarCard({
  ...
  useEffect(() => {
    ...
    const maxBottom = typeof window !== "undefined" ? window.innerHeight - 180 : 400;
  ...
  return (
    <div
      className={cn(
        "fixed z-30 shadow-xl rounded-2xl overflow-visible border transition-colors duration-200",
    ...
    >
      {editMode && onRemove && (
        <button
          ...
          className="absolute -right-2 -top-2 z-20 flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-white shadow-md hover:bg-red-600"
